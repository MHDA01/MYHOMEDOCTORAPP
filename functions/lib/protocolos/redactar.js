...
exports.SISTEMA = exports.RAZONES_EXCLUSION = exports.SECCIONES = exports.MODELO_REDACTOR = void 0;
exports.construirUsuario = construirUsuario;
exports.redactar = redactar;
const gemini_1 = require("./gemini");
exports.MODELO_REDACTOR = "gemini-pro-latest";
exports.SECCIONES = {
...
exports.SISTEMA = `Eres un asistente de extracción documental para un equipo médico. NO eres una fuente de conocimiento.
...
2. Si la excluyes, "razon" debe ser una de: ${exports.RAZONES_EXCLUSION.join(", ")}.
3. Si la incluyes, genera uno o más ítems. Cada ítem:
   - "seccion": una de ${Object.keys(exports.SECCIONES).join(", ")}.
...
function construirUsuario(motivo, recs) {
    const esquema = {
        decisiones: [{
    ...
    const secciones = Object.entries(exports.SECCIONES).map(([k, v]) => `- ${k}: ${v}`).join("\n");
    const inventario = recs.map((r) => `[${r.rec_id}] (página ${r.pagina}) ${r.texto}`).join("\n\n");
    return (`MOTIVO DE CONSULTA: ${motivo.motivo} en ${motivo.edad} (curso de vida: ${motivo.curso_de_vida}).\n\n` +
        `SECCIONES DEL PROTOCOLO:\n${secciones}\n\n` +
        `FORMATO DE SALIDA (JSON):\n${JSON.stringify(esquema, null, 2)}\n\n` +
        `INVENTARIO (${recs.length} recomendaciones):\n${inventario}`);
}
async function redactar(motivo, recs, limiteMs) {
    const { texto, uso } = await (0, gemini_1.llamarGemini)(exports.MODELO_REDACTOR, exports.SISTEMA, construirUsuario(motivo, recs), { limiteMs });
    return { respuesta: texto, uso };
}
//# sourceMappingURL=redactar.js.map