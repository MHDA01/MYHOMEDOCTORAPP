 ...
 */
const IDP_PROMPT =
  'Actúa como un extractor de datos médicos. Analiza este documento. ' +
 ...
 */
async function hardDelete(bucketName: string, filePath: string): Promise<void> {
  try {
...
export const procesarDocumentoMedico = functions
  .region("us-central1")
  .storage.object()
  .onFinalize(async (object) => {
    ...
    if (!tutorId || !patientId || !firestoreDocId) {
      console.error(
        `[IDP] Metadatos incompletos (tutorId/patientId/firestoreDocId) en: ${filePath}`
      );
    ...
    const docRef =
      patientId === tutorId
        ...
        : db.doc(
            `Cuentas_Tutor/${tutorId}/Integrantes/${patientId}/Documentos/${firestoreDocId}`
          );
    ...
    try {
      ...
      if (!apiKey) throw new Error("GEMINI_API_KEY no configurada en el entorno.");
      ...
      try {
        await docRef.update({
          idpStatus: "error",
          idpError:  err?.message ?? String(err),
        });
      } catch (updateErr) {
